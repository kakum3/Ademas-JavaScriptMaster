...
const IDBRequest = indexedDB.open("kakumbase", 1);
...
document.getElementById('add').addEventListener("click",()=>{
  ...
  if(nombre.length > 0){
    if(document.querySelector(".posible") != undefined){
      if(confirm("Hay elementos sin guardar:Quieres guardarlos?")){
        addObjeto({nombre})
        leerObjetos()
      }
    }else{
...
const borrarrObjeto = key => {
  const IDBData = getIDBData('readwrite',"objeto borrado correctamente");
  IDBData.delete(key);
